...
namespace nicorueda
{
    ...
    public class EnemyAttack : MonoBehaviour
    {
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            {
                // 1. Hacer daño (Esto ya lo tenías)
                other.GetComponent<CharacterBase>().TakeDamage(damage, transform.position);

                /*if (attackType == AttackType.Projectile && destroyOnHit)
                {
                    Destroy(gameObject);
                }*/

                // --- 2. ¡NUEVO! AVISAR AL CEREBRO ---
                if (manager != null && manager.AI != null)
                {
                    manager.AI.OnAttackHit();
                }
            }
        }
    }
